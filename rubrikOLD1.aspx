...
function hapusrubrik(id)
{
	...
	if (confirm("Apakah anda ingin hapus data ?") == true) 
	{
		...
		if (tenure == "123") {
			document.getElementsByName("delrubid")[0].value = id;
...
function gokelompok()
{
	document.getElementsByName("statusbaru")[0].value = 1;
	document.forms["barufrm"].submit();
}
</script>
...
function newrubrik()
{
	...
	var kelompok = document.getElementById("soflow12").value;
	var newrub = document.getElementsByName("barurubrik")[0].value;
	var div1 = document.getElementsByName("barudiv")[0].value;
	
	newrub = div1 + "/" + newrub;
	
	var linkx = "getnewrubrikgo.aspx?kelid="+kelompok+"&rubrik="+newrub;
//alert(linkx);
	xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if (outx == 1 || outx == "1")
			{
				...
				//document.getElementById("soflow12").value = kelompok;
...
<script>
function hapusrubrik(rubrikid, rubrix)
{
	if (confirm("Apakah anda ingin hapus rubrik " + rubrix + " ?") == true) 
	{
	//var tahun = document.getElementById("soflow21").value;

		var spkid = document.getElementsByName("spkid")[0].value;
		
		var linkx = "delrubrikgo.aspx?rubrikid="+rubrikid+"&rubrik="+rubrix;
...
<script>
function delrubrik(rubrikid, rubrik)
{
	if (confirm("Apakah anda ingin hapus rubrik " + rubrik + " ?") == true) 
	{
	//var tahun = document.getElementById("soflow21").value;
		var kelompok = document.getElementById("soflow12").value;
		var linkx = "delrubrikgo.aspx?rubrikid="+rubrikid+"&rubrik="+rubrik;
		xhttp = new XMLHttpRequest();
		xhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				//document.getElementById("txtHint").innerHTML = this.responseText;
...
<%
	...
	Dim jmlklp as Integer = 0
	Dim klpidx(100) as Integer
	Dim klpnamax(100) as String
	Dim ridx(1000) as Integer
	Dim rklpidx(1000) as Integer
	Dim rdividx(1000) as Integer
	Dim rubrikx(1000) as String
	Dim rnamaklpx(1000) as String
	Dim rnamadivx(1000) as String
	
	Dim kel8 as Integer = 0
	Dim rubrikxy(1000) as String
	Dim rubrikidxy(1000) as Integer
	Dim jmlrubrikxy as Integer = 0
	Dim keloz as Integer = 0
...
<%			
			...
			If NOT Request.Form("barukelompok") is Nothing Then
				kel8 = Val(Request.Form("barukelompok"))
			End If
			...
			If NOT Request.Form("kelompok") is Nothing Then
				keloz = Val(Request.Form("kelompok"))
			End If
...
'==========================================================================
			SQL0 = "SELECT * FROM [klp] WHERE klp_div=1 ORDER BY klp_nama ASC"
			myCmd.CommandText = SQL0
			myConn.Open()
			myReader = myCmd.ExecuteReader()
			b = 0
            ...
            If myReader.HasRows Then
                While myReader.Read()
					b = b + 1
					klpidx(b) = myReader("klp_id").ToString()
					klpdividx(b) = myReader("klp_div").ToString()
					klpnamax(b) = myReader("klp_nama").ToString()
				End While
			End If
			myConn.Close()
			jmlklp = b
'==========================================================================
' Cari Daftar All Rubrik
'==========================================================================
			'SQL0 = "SELECT klp.ID AS ID, divisi.div_id AS divid, rubrik.kelompok AS klpID, klp.klp_nama AS namaklp, " & _
			'		"klp.klp_div, divisi.div_id AS divid, divisi.div_nama AS namadiv , klp.klp_ket AS klpket " & _
			'		"FROM divisi INNER JOIN klp ON divisi.div_id = klp.klp_div ORDER BY klp.klp_nama ASC"
					
			SQL0 = 	"SELECT rubrik.ID as ID, rubrik.rubrik AS rubrik, rubrik.kelompok AS klpID, klp.klp_nama AS namakel, divisi.div_nama AS namadiv, " & _
					"divisi.div_id AS divid FROM klp INNER JOIN " & _
					"rubrik ON klp.klp_id = rubrik.kelompok INNER JOIN " & _
					"divisi ON klp.klp_div = divisi.div_id " & _
					"WHERE divisi.div_id=1"
			...
			If statusf = 1 Then
				SQL0 = 	"SELECT rubrik.ID as ID, rubrik.rubrik AS rubrik, rubrik.kelompok AS klpID, klp.klp_nama AS namakel, divisi.div_nama AS namadiv, " & _
						...
						"rubrik ON klp.klp_id = rubrik.kelompok INNER JOIN " & _
						"divisi ON klp.klp_div = divisi.div_id " & _
						"WHERE (divisi.div_id=1 AND rubrik.kelompok=" & keloz & ")"
			End If
			If keloz = 0 Then
				SQL0 = 	"SELECT rubrik.ID as ID, rubrik.rubrik AS rubrik, rubrik.kelompok AS klpID, klp.klp_nama AS namakel, divisi.div_nama AS namadiv, " & _
						"divisi.div_id AS divid FROM klp INNER JOIN " & _
...
'response.write(SQL0 & "<br>")
            ...
            If myReader.HasRows Then
                While myReader.Read()
					b = b + 1
					ridx(b) = myReader("ID").ToString()
					rubrikx(b) = myReader("rubrik").ToString()
					rklpidx(b) = myReader("klpID").ToString()
					rdividx(b) = myReader("divid").ToString()
					rnamaklpx(b) = myReader("namakel").ToString()
					rnamadivx(b) = myReader("namadiv").ToString()
				End While
			End If
			myConn.Close()
			jmlrubrik = b
...
'response.write(SQL0 & "<br>")
'==========================================================================
' Cari Daftar Rubrik utk Kelompok
'==========================================================================
			SQL0 = 	"SELECT * FROM rubrik WHERE kelompok=" & kel8
'response.write(SQL0 & "<br>")
			myCmd.CommandText = SQL0
			myConn.Open()
			myReader = myCmd.ExecuteReader()
            ...
            If myReader.HasRows Then
                While myReader.Read()
					b = b + 1
					rubrikidxy(b) = myReader("ID").ToString()
					rubrikxy(b) = myReader("rubrik").ToString()
				End While
			End If
			myConn.Close()
			jmlrubrikxy = b
...
<table style="text-align: left; margin-left: 270px; margin-right: auto; margin-top: auto;" width="950px" border="0" cellspacing="0" cellpadding="0">
	...
	<tr height="20px">
		<td colspan="4" style="padding-left: 0px; text-align: left; font-size: 1.5em">
			<FONT face="Arial" color="#666666">DAFTAR RUBRIK</FONT>
		</td>
	</tr>
	<tr height="30px">
		<td colspan="4" style="padding-left: 20px; text-align: left; font-size: 1.0em">
...
<form method="post" name="barufrm" id="barufrm" action="rubrik.aspx" enctype="multipart/form-data">
<%
...
%>
	...
	<tr height="20px">
		<td colspan="4" style="padding-left: 0px; text-align: left; font-size: 1.3em">
			<FONT face="Arial" color="#44AA44">>> RUBRIK BARU</FONT>
		</td>
	</tr>
	<tr height="40px">
		<td colspan="4" style="padding-left: 20px; text-align: left; font-size: 1.0em">
	...
	<tr height="35px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle" width="700px">
			...
			<select STYLE="color: #000000; background-color: #ffffff; font-size: 0.9em; width: 100px; height: 26px;" id="soflow11" name="barudivisi" onchange="getkel(this.value);" data-placeholder="Jenis" tabindex="1" disabled> 
<%
			for b=1 to jmldiv
%>
...
%>
	...
	<tr height="35px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
			<select STYLE="color: #000000; background-color: #ffffff; font-size: 0.9em; width: 100px; height: 26px;" id="soflow12" name="barukelompok" OnChange="gokelompok();" data-placeholder="Kelompok" tabindex="1"> 
			<option value="" >&nbsp;&nbsp;</option>
<%
			for b=1 to jmlklp
%>
				<option value="<% response.write(klpidx(b)) %>" 
<%
				If klpidx(b)=kel8 Then
					response.write("selected")
				End If
%>
><% response.write(klpnamax(b)) %></option>
<%
			next b
%>
			</select>
	...
	<tr height="35px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
<%
					response.write(namadivx(1))
%>			
				/&nbsp;<input style="font-family: Arial; font-size: 0.9em; width: 50px; height: 18px;" type="text" id="barurubrik" name="barurubrik" class="textbox" tabindex="2">&nbsp;/
				&nbsp;&nbsp; <a id="simpanbtn" href="#" STYLE="font-size: 0.9em;" class="medium button green" onclick="newrubrik();"><i class="fa fa-plus" aria-hidden="true"></i>&nbsp;&nbsp;Tambahkan</a>
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em">										
		</td>
	...
	<tr height="35px">
		...
		<td style="padding-left: 0px; text-align: left; font-size: 0.9em; vertical-align: middle">
			<FONT face="Arial" color="#000">
				<table bordercolor="#EFEFEF" style="text-align: left; margin-left: 0px; margin-right: auto; margin-top: 0;" width="200px" border="1" cellspacing="0" cellpadding="0">
					<tr height="35px" style="background-color: #B0F9F0">
						<td style="padding-left: 8px; text-align: left; font-size: 1.0em;" width="150px">
							DAFTAR RUBRIK
						</td>
						<td style="padding-left: 8px; text-align: left; font-size: 1.0em;" width="50px">
							HAPUS
						</td>
					</tr>
<%
				for c = 1 to jmlrubrikxy	
%>
					<tr height="30px" style="background-color: #FFFFFF">
						<td style="padding-left: 8px; text-align: left; font-size: 1.0em">
<%
							response.write(rubrikxy(c))
%>
						</td>
						<td style="padding-left: 8px; text-align: left; font-size: 1.0em">
							<img id="nomor5" STYLE="vertical-align: middle" src="./images/delete4.png" OnClick="delrubrik(<% Response.Write (rubrikidxy(c)) %>, '<% Response.Write (rubrikxy(c)) %>');" height="26px" width="25px"/>
						</td>
					</tr>
<%
				next c
%>
	...
	<tr height="30px">
		<td colspan="4">
			...
			<input type="hidden" id="barudiv" name="barudiv" value="<%	response.write(namadivx(1)) %>">
			<input type="hidden" id="barucaset" name="barucaset" value="0">
			<input type="hidden" id="editfile" name="editfile">
			<input type="hidden" id="statusbaru" name="statusbaru">
		</td>
	</tr>
</table>
</form>
...
<form method="post" name="filterfrm" id="filterfrm" action="rubrik.aspx" enctype="multipart/form-data">
<%
	If statusbaru=1 Then
%>
<table id="filtertab" style="text-align: left; margin-left: 270px; margin-right: auto; margin-top: auto; display: none" width="730px" border="0" cellspacing="0" cellpadding="0" bgcolor="#CAFFEB">
<%
	Else
%>
<table id="filtertab" style="text-align: left; margin-left: 270px; margin-right: auto; margin-top: auto;" width="730px" border="0" cellspacing="0" cellpadding="0" bgcolor="#CAFFEB">
<%
	End If
%>
	<tr height="20px">
	...
	<tr height="35px">
		<td style="padding-left: 0px; padding-left: 30px; text-align: left; font-size: 0.9em; vertical-align: middle" width="150px">
			<FONT face="Arial" color="#000">
				Kelompok
			</FONT>											
		</td>
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle" width="500px">
			<FONT face="Arial" color="#000">
			<select STYLE="color: #000000; background-color: #ffffff; font-size: 0.9em; width: 100px; height: 26px;" id="soflow21" name="kelompok" data-placeholder="Kelompok" tabindex="1"> 
				<option value="" >&nbsp;&nbsp;</option>
<%
			for b=1 to jmlklp
%>
				<option value="<% response.write(klpidx(b)) %>" ><% response.write(klpnamax(b)) %></option>
<%
			next b
%>
			</select>
...
<%
	If statusbaru=1 Then
%>
<table id="barubtntab" style="margin-left: 270px; display: none" width="570px" align="center" cellpadding="2" cellspacing="2" border="0" bgcolor="#EEE" >
<%
	Else
%>
...
'response.write("---------------------------------------------------------" & pageno & "--" & hal & "   start:" & startval & "-" & endval)
	...
	If statusbaru=1 Then
%>
<table id="datatab" style="margin-left: 270px; display: none" align="center" cellpadding="2" cellspacing="2" border="0" bgcolor="#FAFAFA" >
<%
	Else
%>
...
%>
	<tr height="42px" align="left" style="background-color:#D5D5D5;color:Black;" >
		...
		<td style="font-family: Arial; font-size: 0.9em;text-align: center;" width="100px"><b>RUBRIK</b></td>
...
Try
	...
	for i = startval to endval
		...
		'datastrxx = ""
		datastrxx = ridx(i) & ",'" & rubrikx(i) & "'," & klpdividx(i) & "," & rdividx(i) & ",'" & namaklpx(i) & "'"
...
%>
		...
		<td style="padding-left: 10px; padding-top: 5px;  font-family: Calibri; font-size: 1.1em;text-align: left; vertical-align: top; line-height: 110%"><% Response.Write (rubrikx(i).ToUpper()) %></td>
		<td style="padding-left: 10px; padding-top: 5px;  font-family: Calibri; font-size: 1.1em;text-align: left; vertical-align: top; line-height: 110%"><% Response.Write (rnamaklpx(i).ToUpper()) %></td>
		<td style="padding-left: 5px; padding-top: 5px;  font-family: Calibri; font-size: 1.1em;text-align: center; vertical-align: top; line-height: 110%"><% Response.Write (rnamadivx(i)) %></td>
<%
		If levelaksesq > 1 Then
%>
		<td style="padding-left: 0px; padding-top: 5px; font-family: Calibri; font-size: 0.9em;text-align: center; vertical-align: top;">
			...
			<img id="uploadPreview<% Response.Write (i)%>" src="./images/delete4.png" OnClick="hapusrubrik(<% Response.Write (ridx(i)) %>, '<% Response.Write (rubrikx(i)) %>');" height="26px" width="25px"/><br/>
		</td>
<%
		End If
%>
...
<form method="post" name="delfrm" id="delfrm" action="rubrikdelgo.aspx" enctype="multipart/form-data">
	<input type="hidden" id="delrubid" name="delrubid">
</form>
