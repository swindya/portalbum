...
<script>
function getnomorpks()
{

	var tahun = document.getElementById("edittahun").value;
	var tgl = document.getElementsByName("edittgl")[0].value;
	var pksid = document.getElementsByName("pksid")[0].value;
	var userid = document.getElementsByName("mcuserid")[0].value;
	var mohonid = document.getElementsByName("padaid")[0].value;
	var jmlnego = document.getElementsByName("jmlnego")[0].value;
	
	var linkx = "getnomorpksgo.aspx?mohonid="+mohonid+"&pksid="+pksid+"&tgl="+tgl+"&tahun="+tahun+"&jmlnego="+jmlnego;
//alert(linkx);
...
<script>
function hapusnomorpks(mohonid, nomorpks, noke)
{
	...
	//var tahun = document.getElementById("soflow21").value;
		...
		var linkx = "delnomorpksgo.aspx?mohonid="+mohonid+"&pksid="+pksid+"&nomor="+nomorpks+"&nomorke="+noke;
//alert(linkx);
		xhttp = new XMLHttpRequest();
		xhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
...
<%
    ...
    'results As String
	Dim a, aaa, b, bb, c, d, g, jmla, jmldata as integer
	Dim user0 as string
	Dim pwd0 as string
	...
	Dim nomorx2 as String = ""
	Dim nomorx3 as String = ""
	Dim nomorx4 as String = ""
	Dim nomorx5 as String = ""
	Dim nomorx6 as String = ""
...
'---------------------------------------------------------------------------------------
			...
			End If
			If mohonid = 0 Then
				If NOT Request.Form("pksadaid") is Nothing Then
					madaid = Request.Form("pksadaid")
					mohonid = Val(madaid)
					adaid = mohonid
				End If
			End If
'---------------------------------------------------------------------------------------
			...
			End If
'---------------------------------------------------------------------------------------
			If NOT Request.Form("pksnext") is Nothing Then
				statusnext = Val(Request.Form("pksnext"))
			End If
...
'==========================================================================
            ...
            If myReader.HasRows Then
                While myReader.Read()
					b = b + 1
					rubrikx(b) = myReader("rubrik").ToString() & "SPK/"
				End While
			End If
			myConn.Close()
			jmlrubrik = b
...
' BACA DATA DIVISI
'==============================================================================
			SQL3 = "SELECT * FROM divisi WHERE ID=" & divisix
'response.write(SQL3 & "<br>")
			myCmd.CommandText = SQL3
			myConn.Open()
			vendorx = ""
...
'response.write(SQL3 & "<br>")
			...
			myConn.Close()
'==============================================================================
' BACA DATA NEGOSIASI -> HARGA NEGO
'==============================================================================
			...
			If myReader.HasRows Then
				While myReader.Read()
					jmlnegox = Val(myReader("jmlpostnego").ToString())
...
'==============================================================================
			...
			g = 0
			If vendorx.length() > 3 Then
				g = g + 1
				vendorz(g) = vendorx
			End If
			If vendorx2.length() > 3 Then
				g = g + 1
				vendorz(g) = vendorx2
			End If
			If vendorx3.length() > 3 Then
				g = g + 1
				vendorz(g) = vendorx3
			End If
			If vendorx4.length() > 3 Then
				g = g + 1
				vendorz(g) = vendorx4
			End If
			If vendorx5.length() > 3 Then
				g = g + 1
				vendorz(g) = vendorx5
			End If
			If vendorx6.length() > 3 Then
				g = g + 1
				vendorz(g) = vendorx6
			End If
			jmlg = g
'response.write(SQL3 & "<br>")
'==============================================================================
' Cari apakah data SKP dengan pengadaanID sudah ada. Jika belum masukkan data
'==============================================================================
...
'response.write(vendorx & "<br>")
			...
			If a = 0 Then
				SQLjo = "SELECT MAX(nourut) AS maxnourut FROM [surat_pks] WHERE (nomor LIKE '%" & divstr & "%' AND tahun=" & tahunx & ")"
				maxu = 0
...
'---------------------------------------------------------------------------
				...
				For c = 1 to jmlg
					maxu = maxu + 1
					nomorxx = maxu & "/PKS/" & namadiv & "/" & tahunx
					nomorzz(c) = nomorxx
					SQL3 = "INSERT INTO surat_pks(pengadaanID, tglpks, namapengadaan, jenisada, nourut, nomor, divisi, tahun, nominal, " & _
							"vendor, createduser, createddate) VALUES(" & mohonid & ",GETDATE(),'" & namax.toUpper() & "','" & jenisadax & "'," & _
							maxu & ",'" & nomorxx & "',1," & tahunx & "," & jmlnegox & ",'" & vendorz(c) & "'," & userid & ",GETDATE())"
					myCmd.CommandText = SQL3
					myConn.Open()
					myReader = myCmd.ExecuteReader()
					myConn.Close()
				Next c
'---------------------------------------------------------------------------
				SQL4 = "INSERT INTO pengadaan_pks(pengadaanID, tgl, perihal, nomor, nomor2, nomor3, nomor4, nomor5, nomor6, vendor, " & _
						"vendor2, vendor3, vendor4, vendor5, vendor6, createduser, createddatetime) VALUES(" & _
						mohonid & ",GETDATE(),'" & namax.toUpper() & "','" & nomorzz(1) & "','" & nomorzz(2) & "','" & nomorzz(3) & _
						"','" & nomorzz(4) & "','" & nomorzz(5) & "','" & nomorzz(6) & "','" & vendorz(1).toUpper() & "','" & vendorz(2).toUpper() & _
						"','" & vendorz(3).toUpper() & "','" & vendorz(4).toUpper() & "','" & vendorz(5).toUpper() & "','" & vendorz(6).toUpper() & "'," & userid & ",GETDATE())"
				myCmd.CommandText = SQL4
				myConn.Open()
				myReader = myCmd.ExecuteReader()
				myConn.Close()
'---------------------------------------------------------------------------				
				SQL1 = "UPDATE pengadaan SET step='PKS', tglstep='" & tglkux & "' WHERE ID=" & mohonid
				myCmd.CommandText = SQL1
				myConn.Open()
				myReader = myCmd.ExecuteReader()
				myConn.Close()
			Else If a > 0 Then
				SQL4 = "UPDATE pengadaan_pks SET vendor='" & vendorz(1).toUpper() & "', vendor2='" & vendorz(2).toUpper() & "', vendor3='" & vendorz(3).toUpper() & _
						"', vendor4='" & vendorz(4).toUpper() & "', vendor5='" & vendorz(5).toUpper() & "', vendor6='" & vendorz(6).toUpper() & _
						"' WHERE pengadaanID=" & mohonid
				myCmd.CommandText = SQL4
				myConn.Open()
				myReader = myCmd.ExecuteReader()
				myConn.Close()
...
'==============================================================================
			...
			If myReader.HasRows Then
				While myReader.Read()
					pksidx = Val(myReader("ID").ToString())
...
<table style="text-align: left; margin-left: 270px; margin-right: auto; margin-top: auto;" width="950px" border="0" cellspacing="0" cellpadding="0">
	...
	<tr height="20px">
		<td colspan="4" style="padding-left: 0px; text-align: left; font-size: 1.5em">
			<FONT face="Arial" color="#888">PENGADAAN: </FONT>
			<FONT face="Arial" color="#000">PKS</FONT>
		</td>
	</tr>
	<tr height="30px">
		<td colspan="4" style="padding-left: 20px; text-align: left; font-size: 1.0em">
...
<form method="post" name="editfrm" id="editfrm" action="editprosespksgo.aspx" enctype="multipart/form-data">
<table id="dataedittab" style="text-align: left; margin-left: 270px; margin-right: auto; margin-top: 0;" width="1050px" border="0" cellspacing="0" cellpadding="0">
	<tr height="20px">
		<td colspan="4" style="padding-left: 0px; text-align: left; font-size: 1.3em">
			<FONT face="Arial" color="#44AA44">>> REVIEW/UPDATE</b></FONT>
		</td>
	</tr>
	<tr height="40px">
		<td colspan="4" style="padding-left: 20px; text-align: left; font-size: 1.0em">
	...
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle" width="700px">
			<FONT face="Arial" color="#000">
				...
				<input autocomplete="off" STYLE="padding-left:5px; font-size:1.0em; border:1px solid #ababab; height: 25px; width: 100px" autocomplete="off" class="tcal" id="edittgl" name="edittgl" type="text" value="<% response.write(tglxxx) %>" tabindex="1">
			</FONT>
		</td>
	...
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle;">
			<FONT face="Arial" color="#AAAAFF">
				<input style="padding-right:10px; background-color:#FFFFFF; color:#2222FF; font-size: 1.1em; text-align: right; width: 160px; height: 20px;" type="text" id="editnomor" name="editnomor" class="textbox" value="<% response.write(nomorx) %>" tabindex="6" readonly>
			</FONT>
			&nbsp;&nbsp;
<%
			If nomor6x.length()<3 Then
%>
			&nbsp;&nbsp;&nbsp;<a id="nomorbtn" href="#" class="small button blue" STYLE="vertical-align: top;" onclick="getnomorpks();"><i class="fas fa-pen" aria-hidden="true"></i>&nbsp;&nbsp;<i class="fas fa-plus" aria-hidden="true"></i>&nbsp;&nbsp;Buat Nomor</a>
<%
			End If
...
%>
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#AAAAFF">
				<input style="padding-right:10px; background-color:#FFFFFF; color:#2222FF; font-size: 1.1em; text-align: right; width: 160px; height: 20px;" type="text" id="editnomor2" name="editnomor2" class="textbox" value="<% response.write(nomor2x) %>" tabindex="6" readonly>
			</FONT>
<%
			If ab = 2 Then
%>
			<img id="nomor2" STYLE="vertical-align: middle" src="./images/delete4.png" OnClick="hapusnomorpks(<% Response.Write (mohonid) %>, '<% Response.Write (nomor2x) %>', 2);" height="26px" width="25px"/>
<%
			End If
%>
		</td>
...
%>
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#AAAAFF">
				<input style="padding-right:10px; background-color:#FFFFFF; color:#2222FF; font-size: 1.1em; text-align: right; width: 160px; height: 20px;" type="text" id="editnomor3" name="editnomor3" class="textbox" value="<% response.write(nomor3x) %>" tabindex="6" readonly>
			</FONT>
<%
			If ab = 3 Then
%>
			<img id="nomor3" STYLE="vertical-align: middle" src="./images/delete4.png" OnClick="hapusnomorpks(<% Response.Write (mohonid) %>, '<% Response.Write (nomor3x) %>', 3);" height="26px" width="25px"/>
<%
			End If
%>
		</td>
...
%>
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#AAAAFF">
				<input style="padding-right:10px; background-color:#FFFFFF; color:#2222FF; font-size: 1.1em; text-align: right; width: 160px; height: 20px;" type="text" id="editnomor4" name="editnomor4" class="textbox" value="<% response.write(nomor4x) %>" tabindex="6" readonly>
			</FONT>
<%
			If ab = 4 Then
%>
			<img id="nomor4" STYLE="vertical-align: middle" src="./images/delete4.png" OnClick="hapusnomorpks(<% Response.Write (mohonid) %>, '<% Response.Write (nomor4x) %>', 4);" height="26px" width="25px"/>
<%
			End If
%>
		</td>
...
%>
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#AAAAFF">
				<input style="padding-right:10px; background-color:#FFFFFF; color:#2222FF; font-size: 1.1em; text-align: right; width: 160px; height: 20px;" type="text" id="editnomor5" name="editnomor5" class="textbox" value="<% response.write(nomor5x) %>" tabindex="6" readonly>
			</FONT>
<%
			If ab = 5 Then
%>
			<img id="nomor5" STYLE="vertical-align: middle" src="./images/delete4.png" OnClick="hapusnomorpks(<% Response.Write (mohonid) %>, '<% Response.Write (nomor5x) %>', 5);" height="26px" width="25px"/>
<%
			End If
%>
		</td>
...
%>
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#AAAAFF">
				<input style="padding-right:10px; background-color:#FFFFFF; color:#2222FF; font-size: 1.1em; text-align: right; width: 160px; height: 20px;" type="text" id="editnomor6" name="editnomor6" class="textbox" value="<% response.write(nomor6x) %>" tabindex="6" readonly>
			</FONT>
			<img id="nomor6" STYLE="vertical-align: middle" src="./images/delete4.png" OnClick="hapusnomorpks(<% Response.Write (mohonid) %>, '<% Response.Write (nomor6x) %>', 6);" height="26px" width="25px"/>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em">
		</td>
	</tr>
...
%>
	<tr height="38px">
		<td style="padding-top: 10px; padding-left: 0px; text-align: left; font-size: 1.0em; vertical-align: middle">
			<FONT face="Arial" color="#000">
				Nama Vendor <% response.write(xy) %>
			</FONT>											
		</td>
		<td style="padding-top: 10px; padding-left: 0px; text-align: center; font-size: 1.0em; vertical-align: middle">
			<FONT face="Arial" color="#000">
				 :
			</FONT>											
		</td>
		<td style="padding-top: 10px; padding-left: 0px; text-align: left; font-size: 1.0em;  vertical-align: middle">
			<FONT color="#000">
<% 
				response.write(vendorx & " (Rp. " & jmlpost.ToString("#,###") & ")") 
%>
			</FONT>
		</td>
		<td style="padding-top: 10px; padding-left: 0px; text-align: center; font-size: 1.0em">										
		</td>
	</tr>
<%
		End If
...
%>
	<tr height="38px">
		<td style="padding-left: 30px; padding-top: 0px; text-align: left; font-size: 1.0em; vertical-align: middle">
			<FONT face="Arial" color="#000">
				Nama Vendor <% response.write(xy) %>
			</FONT>											
		</td>
		...
		<td style="padding-top: 0px; padding-left: 0px; text-align: left; font-size: 1.0em;  vertical-align: middle">
			<FONT color="#000">
<% 
				response.write(vendorx2 & " (Rp. " & jmlpost2.ToString("#,###") & ")") 
%>
			</FONT>
		</td>
...
%>
	<tr height="38px">
		...
		<td style="padding-top: 0px; padding-left: 0px; text-align: left; font-size: 1.0em;  vertical-align: middle">
			<FONT color="#000">
<% 
				response.write(vendorx3 & " (Rp. " & jmlpost3.ToString("#,###") & ")") 
%>
			</FONT>
		</td>
		<td style="padding-top: 8px; padding-left: 0px; text-align: center; font-size: 1.0em">										
...
%>
	<tr height="38px">
		...
		<td style="padding-top: 0px; padding-left: 0px; text-align: left; font-size: 1.0em;  vertical-align: middle">
			<FONT color="#000">
<% 
				response.write(vendorx4 & " (Rp. " & jmlpost4.ToString("#,###") & ")") 
%>
			</FONT>
		</td>
		<td style="padding-top: 8px; padding-left: 0px; text-align: center; font-size: 1.0em">										
...
%>
	<tr height="38px">
		...
		<td style="padding-top: 0px; padding-left: 0px; text-align: left; font-size: 1.0em;  vertical-align: middle">
			<FONT color="#000">
<% 
				response.write(vendorx5 & " (Rp. " & jmlpost5.ToString("#,###") & ")") 
%>
			</FONT>
		</td>
		<td style="padding-top: 8px; padding-left: 0px; text-align: center; font-size: 1.0em">										
...
%>
	<tr height="38px">
		...
		<td style="padding-top: 0px; padding-left: 0px; text-align: left; font-size: 1.0em;  vertical-align: middle">
			<FONT color="#000">
<% 
				response.write(vendorx6 & " (Rp. " & jmlpost6.ToString("#,###") & ")") 
%>
			</FONT>
		</td>
		<td style="padding-top: 8px; padding-left: 0px; text-align: center; font-size: 1.0em">										
...
<%
		response.write("Rp.  " & jmlnegox.ToString("#,###"))
%>
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em">										
...
<%
						If NOT file2x is Nothing Then
							If file2x.length()>1 Then
								filestrx = "." & folderx & file2x
							Else
								filestrx = "./images/nofile.png"
							End If
%>
...
<table id="btntab" style="text-align: left; margin-left: 270px; margin-right: auto; margin-top: 0;" width="880px" border="0" cellspacing="0" cellpadding="0">
	<tr height="20px">
		<td colspan="5" style="padding-left: 0px; text-align: left;">
			<Input id="pksid" name="pksid" type="hidden" value="<% response.write(pksidx) %>">
			<Input id="jmlnego" name="jmlnego" type="hidden" value="<% response.write(jmlnegox) %>">
