...
function gotopoc()
{
	var mystep = document.getElementById("soflow24").value;
	if (mystep == "POC")
	{
		document.forms["gotopocfrm"].submit();
	}
	else if (mystep == "EVAL")
	{
	...
	{
		alert("Pilih PoC atau Eval terlebih dahulu.");
		return false;
	}
}
</script>
...
<%
	...
	Dim mydate as String = regDate.ToString("yyyy-MM-dd")
...
<%
			...
			End If
			If mohonid = 0 Then
				If NOT Request.Form("aanadaid") is Nothing Then
					 madaid = Request.Form("aanadaid")
					 mohonid = Val(madaid)
				End If
			End If
			adaid = mohonid
'-----------------------------------------------------------------------------------
			If NOT Request.Form("aannext") is Nothing Then
				statusnext = Cint(Request.Form("aannext"))
			End If
			If statusnext = 0 Then
				If NOT Request.Form("statusmohon") is Nothing Then
					 statusnext = Cint(Request.Form("statusmohon"))
				End If
			End If
			If statusnext = 0 Then
...
'-----------------------------------------------------------------------------------			
			...
			If userid=0 OR Len(userid)<1 Then
				userid = idq
			End If
'response.write("OK")
...
'==============================================================================
' Cari apakah data aanwijzing dengan pengadaanID sudah ada. Jika belum masukkan data
'==============================================================================
			SQL3 = "SELECT * FROM pengadaan_aanwijzing WHERE pengadaanID=" & mohonid
'response.write(SQL3 & "<br>")
			myCmd.CommandText = SQL3
			myConn.Open()
			myReader = myCmd.ExecuteReader()
			...
			myConn.Close()			
			If a=0 Then
				SQL4 = "INSERT INTO pengadaan_aanwijzing(pengadaanID, klp, divisi, createduser, tgl, createddatetime) VALUES(" & mohonid & "," & _
						klpx & "," & divisix & "," & userid & ",GETDATE(),GETDATE())"
'response.write(SQL4)
				...
				SQL1 = "UPDATE pengadaan SET step='AANWIJZING', tglstep='" & mydate & "' WHERE ID=" & mohonid
...
'response.write("ABC")
'==============================================================================
' BACA DATA PENGADAAN AANWIJZING
'==============================================================================
			SQL3 = "SELECT * FROM pengadaan_aanwijzing WHERE pengadaanID=" & mohonid
'response.write("Tiga")
			tglaanxx = ""
...
'response.write(tglaanx & "<br>")
					If Not tglaanx is nothing AND tglaanx.length() > 3 Then
						'Dim tglstr() = tglaanx.Split("-")
						'Dim tglstr() = tglstr1(0).Split("/")
						'tglaanxx = tglstr(2) & "-" & tglstr(1) & "-" & tglstr(0)
...
<table style="text-align: left; margin-left: 270px; margin-right: auto; margin-top: auto;" width="950px" border="0" cellspacing="0" cellpadding="0">
	...
	<tr height="20px">
		<td colspan="4" style="padding-left: 0px; text-align: left; font-size: 1.5em">
			...
			<FONT face="Arial" color="#000">AANWIJZING</FONT>
		</td>
	</tr>
	<tr height="30px">
		<td colspan="4" style="padding-left: 20px; text-align: left; font-size: 1.0em">
...
<form method="post" name="editfrm" id="editfrm" action="editprosesaanwijgo.aspx" enctype="multipart/form-data">
...
%>
	...
	<tr height="35px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: top; font-size: 1.0em;">
			<FONT face="Arial" color="#000">
<%
		response.write("Rp.  " & nilaihpsxx.ToString("#,###"))
%>
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em">										
	...
	<tr height="65px">
		...
		<td style="padding-left: 0px; font-size:1.0em; text-align: left; vertical-align: top">
			<FONT color="#000">
				<textarea STYLE="font-family: Arial; color: #000000; padding-left: 5px; border:1px solid #ababab; font-size:1.0em; line-height: 110%; height: 50px; width: 680px;" id="editketerangan" name="editketerangan" rows="3" cols="81" tabindex="9"><% response.write(ketx) %></textarea>
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em">										
		</td>
...
<table id="btntab" style="text-align: left; margin-left: 270px; margin-right: auto; margin-top: 0;" border="0" cellspacing="0" cellpadding="0">
	<tr height="50px">
		<td colspan="6" style="padding-left: 0px; text-align: left;">
			<Input id="aanid" name="aanid" type="hidden" value="<% response.write(aanidxx) %>">
			...
			<input type="hidden" id="statusnext" name="statusnext" value="<% response.write(statusnext) %>">
		</td>
	</tr>
	<tr height="40px">
...
<%
		If statusnext=0 Then
%>
			<a href="#" onclick="gotopoc();"><img src="./images/nextblue.jpg" alt="Aanwijzing" style="width:60px;height:60px;"></a>
<%
		End If
%>
		</td>
...
%>
			...
			<FONT face="Arial" color="#000">
			<select STYLE="color: #000000; background-color: #ffffff; font-size: 1.1em; width: 180px; height: 32px;" id="soflow24" name="mystep" data-placeholder="Jenis" tabindex="10"> 
				<option value="" selected>&nbsp;</option>
				<option value="POC">P o C</option>
				<option value="EVAL">Evaluasi Ad & T</option>
			</select>			
			</FONT>
<%
		End If
...
<form name="gotopocfrm" id="gotopocfrm" action="prosespoc.aspx" method="post" enctype="multipart/form-data">
	<input type="hidden" id="mcuserid" name="mcuserid" value="<% Response.Write (userid) %>">
	<input type="hidden" id="pengadaanid" name="pengadaanid" value="<% Response.Write (adaid) %>">
</form>

<form name="gotoevalfrm" id="gotoevalfrm" action="prosesevaluasi.aspx" method="post" enctype="multipart/form-data">
