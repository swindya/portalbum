			...
			If Not ds.Tables(0).Rows(i)(j).ToString() is Nothing Then
				...
				If j = 1 Then
					if datax.length() > 5 Then
						...
						tglx = tglarr(2) & "-" & tglarr(1) & "-" & tglarr(0)
					End If
				Else if j = 2 Then
					jenisx = ds.Tables(0).Rows(i)(j).ToString()
				Else if j = 3 Then
					perihalx = ds.Tables(0).Rows(i)(j).ToString()
				Else if j = 4 Then
					rubrikx = ds.Tables(0).Rows(i)(j).ToString()
				Else if j = 5 Then
					tujuanx = ds.Tables(0).Rows(i)(j).ToString()
				Else if j = 6 Then
					tembusanx = ds.Tables(0).Rows(i)(j).ToString()
				Else if j = 7 Then
					undanganx = ds.Tables(0).Rows(i)(j).ToString()
				End If
