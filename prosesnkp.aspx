...
function gotoeval()
{
	var mystep = document.getElementById("soflow25").value;

	if (mystep == "SANGGAH")
	{
		document.forms["gotosangfrm"].submit();
	}
	else if (mystep == "TUNJUK")
	{
		document.forms["gototunfrm"].submit();
	}
	else
	{
		alert("Pilih Sanggahan atau Penunjukan terlebih dahulu.");
		return false;
	}
}
</script>
...
<script>
function getnomornkp(tahun)
{
	var jenissurat = document.getElementById("soflow24").value;
	jenissurat = jenissurat.toLowerCase();
	if (jenissurat.length < 3)
	{
		alert("Isi field memo/notin terlebih dahulu.");
		return false;
	}
	if (jenissurat=="memo" || jenissurat=="notin")
	{
		//var tahun = document.getElementById("soflow21").value;
		var tgl = document.getElementsByName("edittgl")[0].value;
		var nkpid = document.getElementsByName("nkpid")[0].value;
		...
		var linkx = "getnomornkpgo.aspx?mohonid="+mohonid+"&nkpid="+nkpid+"&perihal="+perihal+"&tgl="+tgl+"&tahun="+tahun+"&jenissurat="+jenissurat;
		//document.getElementsByName("editketerangan")[0].value = linkx;
...
<%
	...
	Dim idxx(100)  as Integer
	Dim tglverxx(100) as String
	Dim jenisverxx(100) as String
	Dim statusxx(100) as String
	Dim nourutxx(100) as String
	Dim ketxx(100) as String
...
<%
			...
			End If
			If mohonid = 0 Then
				If NOT Request.Form("nkpadaid") is Nothing Then
					madaid = Request.Form("nkpadaid")
					mohonid = Val(madaid)
					adaid = mohonid
				End If
			End If
			...
			End If
'-----------------------------------------------------------------------------------
			If NOT Request.Form("nkpnext") is Nothing Then
				statusnext = Val(Request.Form("nkpnext"))
			End If
...
'==============================================================================
			SQL3 = "SELECT * FROM pengadaan_nkp WHERE pengadaanID=" & mohonid
'response.write(SQL3 & "<br>")
			myCmd.CommandText = SQL3
			myConn.Open()
			myReader = myCmd.ExecuteReader()
			...
			If a=0 Then
				SQL4 = "INSERT INTO pengadaan_nkp(pengadaanID, tgl, createduser, createddatetime) VALUES(" & _
...
'response.write(SQL4)
				...
				SQL1 = "UPDATE pengadaan SET step='NKP', tglstep='" & tglkux & "' WHERE ID=" & mohonid
'response.write(SQL1)
...
'==============================================================================
			SQL3 = "SELECT * FROM pengadaannkpverifikasi WHERE pengadaanID=" & mohonid & " ORDER BY ID ASC"
'response.write("---------------------------------------------" & SQL3)
			myCmd.CommandText = SQL3
			myConn.Open()
			myReader = myCmd.ExecuteReader()
			...
			If myReader.HasRows Then
				While myReader.Read()
					a = a+1
					idxx(a) = Val(myReader("id").ToString())
...
<table style="text-align: left; margin-left: 270px; margin-right: auto; margin-top: auto;" width="950px" border="0" cellspacing="0" cellpadding="0">
	...
	<tr height="20px">
		<td colspan="4" style="padding-left: 0px; text-align: left; font-size: 1.5em">
			...
			<FONT face="Arial" color="#000">NOTA KEPUTUSAN PENGADAAN (NKP)</FONT>
		</td>
	</tr>
	<tr height="30px">
		<td colspan="4" style="padding-left: 20px; text-align: left; font-size: 1.0em">
...
'response.write(jenissuratx)
'====================================================================================================
%>

<form method="post" name="editfrm" id="editfrm" action="editprosesnkpgo.aspx" enctype="multipart/form-data">
...
%>
		...
		<td colspan="2" style="padding-left: 0px; text-align: left; vertical-align: top">
			<FONT face="Arial" color="#000">
...
%>		
	...
	<tr height="35px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: top">
			<FONT face="Arial" color="#000">
			<select STYLE="color: #000000; background-color: #ffffff; font-size: 1.0em; width: 150px; height: 28px" id="soflow24" name="editsurat" data-placeholder="Jenis" tabindex="5"> 
				<option value="" >&nbsp;&nbsp;</option>
				<option value="MEMO" &nbsp;
<%
	If jenissuratx.ToUpper()="MEMO" Then
...
>MEMO</option>
				<option value="NOTIN" &nbsp;
<%
	If jenissuratx.ToUpper()="NOTIN" Then
		response.write("selected")
	End If
%>
>NOTA INTERN</option>
	...
	<tr height="40px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#AAAAFF">
				<input style="padding-right:10px; background-color:#FFFFFF; color:#2222FF; font-size: 1.1em; text-align: right; width: 135px; height: 20px;" type="text" id="editnomor" name="editnomor" class="textbox" value="<% response.write(nomornkpx) %>" tabindex="6" readonly>
			</FONT>
			&nbsp;&nbsp;
<%
		'If nomornkpx.length()<3 Then
%>
			<a id="nomorbtn" href="#" class="small button blue" onclick="getnomornkp(<% response.write(tahunx) %>);"><i class="fas fa-pen" aria-hidden="true"></i>&nbsp;&nbsp;Buat Nomor</a>
<%
		'End If
%>
		</td>
...
%>	
	...
	<tr height="65px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: top">
			<FONT face="Arial" color="#000">
				<textarea STYLE="font-family: Arial; color: #000000; padding-left: 5px; border:1px solid #ababab; font-size:1.0em; line-height: 110%; height: 46px; width: 680px;" id="editketerangan" name="editketerangan" tabindex="10"><% response.write(ketx) %></textarea>
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em">										
		</td>
...
%>
		...
		<td style="padding-left: 0px; padding-top: 5px; font-family: Arial; font-size: 0.9em;text-align: center; vertical-align: top;" width="80px">
			<img id="uploadPreview<% Response.Write (i)%>" src="./images/delete4.png" OnClick="hapusver(<% Response.Write (mohonid) %>, <% Response.Write (idxx(b)) %>);" height="26px" width="25px"/>
		</td>
	</tr>
<%
	next b
...
<table id="btntab" style="text-align: left; margin-left: 270px; margin-right: auto; margin-top: 0;" width="880px" border="0" cellspacing="0" cellpadding="0">
	<tr height="60px">
		<td colspan="5" style="padding-left: 0px; text-align: left;">
			<Input id="nkpid" name="nkpid" type="hidden" value="<% response.write(nkpidxx) %>">
...
<%
		If statusnext=0 AND statusverok=1 Then
%>
			<a href="#" onclick="gotoeval();"><img src="./images/nextblue.jpg" alt="" style="width:60px;height:60px;"></a>
<%
		End If
%>
		</td>
...
%>
			<FONT face="Arial" color="#000">
			<select STYLE="color: #000000; background-color: #ffffff; font-size: 1.1em; width: 150px" id="soflow25" name="mystep" data-placeholder="Jenis" tabindex="10"> 
				<option value="" selected>&nbsp;</option>
				<option value="SANGGAH">Sanggahan</option>
				<option value="TUNJUK">Penunjukan</option>
			</select>			
			</FONT>
<%
		End If
...
<form name="verdelfrm" id="verdelfrm" method="post" action="prosesnkpverdelgo.aspx" enctype="multipart/form-data">
...
<form method="post" name="veriffrm" id="veriffrm" action="addprosesnkpvergo.aspx" enctype="multipart/form-data">
...
<form name="gotosangfrm" id="gotosangfrm" action="prosessanggah.aspx" method="post" enctype="multipart/form-data">
	<input type="hidden" id="mcuserid" name="mcuserid" value="<% Response.Write (userid) %>">
	<input type="hidden" id="pengadaanid" name="pengadaanid" value="<% Response.Write (adaid) %>">
</form>

<form name="gototunfrm" id="gototunfrm" action="prosespenunjukan.aspx" method="post" enctype="multipart/form-data">
	<input type="hidden" id="tunuserid" name="tunuserid" value="<% Response.Write (userid) %>">
