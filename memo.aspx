...
function hapusmemo(id)
{
	if (confirm("Apakah anda ingin hapus data ?") == true) 
	{
		...
		if (tenure == "123") {
			document.getElementsByName("delmemoid")[0].value = id;
...
<script>
function editdatax(id, nomemo, tgl, jenis, perihal, rubrik, tujuan, tembusan, jenisundangan, namafile, namafolder, extens, statusbum, wrnama, wrnpp, namaklp, userklp)
{
//alert(id + "-" + rubrik);
	document.getElementById("databarutab").style.display = "none";
...
' Data MEMO
'============================================================================
			SQL00 = "SELECT ID FROM memo WHERE (ID>0 AND klp=" & klpq & perihalstr & tglstr & ") ORDER BY tglmemo DESC"
			If Val(posisiq) <= 4 Then
				SQL00 = "SELECT ID FROM memo WHERE (ID>0 AND klp=" & klpq & perihalstr & tglstr & ") ORDER BY tglmemo DESC"
			Elseif Val(posisiq) >= 5 AND Val(posisiq) <=7 Then
				SQL00 = "SELECT ID FROM memo WHERE (ID>0 AND divisi=" & divq & perihalstr & tglstr & ") ORDER BY tglmemo DESC"
			End If
			If namaklpq.ToUpper() = "BUM" Then
				SQL00 = "SELECT ID FROM [memo] WHERE (ID>0" & perihalstr & tglstr & ") ORDER BY tglmemo DESC"
			End If
			SQLexport = "SELECT * FROM [memo] WHERE (ID>0" & perihalstr & tglstr & ") ORDER BY tahun DESC,nourut DESC"
...
<table style="text-align: left; margin-left: 270px; margin-right: auto; margin-top: auto;" width="950px" border="0" cellspacing="0" cellpadding="0">
	...
	<tr height="20px">
		<td colspan="4" style="padding-left: 0px; text-align: left; font-size: 1.5em">
			<FONT face="Arial" color="#666666">DAFTAR MEMO</FONT>
		</td>
	</tr>
	<tr height="30px">
		<td colspan="4" style="padding-left: 20px; text-align: left; font-size: 1.0em">
...
<form method="post" name="barufrm" id="barufrm" action="memoaddgo.aspx" enctype="multipart/form-data">
<%
...
%>
	...
	<tr height="20px">
		<td colspan="4" style="padding-left: 0px; text-align: left; font-size: 1.3em">
			<FONT face="Arial" color="#44AA44">>> MEMO BARU</b></FONT>
		</td>
	</tr>
	<tr height="40px">
		<td colspan="4" style="padding-left: 20px; text-align: left; font-size: 1.0em">
	...
	<!--tr height="35px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
			<select STYLE="color: #000000; background-color: #ffffff; font-size: 0.9em; width: 150px" id="soflow2" name="barutahun" data-placeholder="Jenis" tabindex="2"> 
				<option value="" >&nbsp;&nbsp;</option>
...
%>
	...
	<tr height="35px">
		...
		<td style="padding-left: 0px; padding-top: 3px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
				<div style="text-align:left;margin-middle:0%">
...
<form method="post" name="editfrm" id="editfrm" action="memoeditgo.aspx" enctype="multipart/form-data">
<%
	If statusbaru=0 Then
%>
...
%>
	...
	<tr height="65px">
		...
		<td style="padding-left: 0px; text-align: center; font-size: 0.9em">										
		</td>
	</tr>
	<tr height="40px">
		<td style="padding-left: 0px; text-align: left; font-size: 0.9em; vertical-align: middle">
	...
	<tr height="30px">
		<td colspan="4">
			<input type="hidden" id="editmemoid" name="editmemoid">
...
<table id="datatab" style="margin-left: 270px;" width="1150px" align="center" cellpadding="2" cellspacing="2" border="0" bgcolor="#FAFAFA" >
	<tr height="42px" align="left" style="background-color:#D5D5D5;color:Black;" >
		...
		<td style="font-family: Arial; font-size: 0.9em;text-align: center;" width="110px"><b>TGL MEMO</b></td>            
		<td style="font-family: Arial; font-size: 0.9em;text-align: center;" width="90px"><b>JENIS</b></td>
		<td style="font-family: Arial; font-size: 0.9em;text-align: center;" width="120px"><b>NO MEMO</b></td>
...
Try
	...
	for i = startval to endval
		ii = ii + k
		bb = bb + 1
		SQL1 = "SELECT * FROM memo WHERE (ID=" & idz(i) & ")"
		'SQL1 = "SELECT * FROM memo WHERE ((ID>=10 AND ID<=50)" & _
		'		") ORDER BY tglmemo DESC"
		...
		If myReader.HasRows Then
			While myReader.Read()
				...
				nomemoxx = myReader("nomor").ToString()
				nourutxx = myReader("nourut").ToString()
				tglxx = myReader("tglmemo").ToString()
		...
		End If
		myConn.Close()
		If tglxx.length() < 6 OR tglxx is Nothing Then
			tglxx = tglinputxx
		End If
'================================================================================
		...
		myConn.Close()
'================================================================================		
		nomemoxx = rubrikxx & nourutxx
		
		SQL1 = "SELECT * FROM jenismemo WHERE (js_id=" & jenisxx & ")"	
		myCmd.CommandText = SQL1
		myConn.Open()
		myReader = myCmd.ExecuteReader()
		
		Dim jenismemoxx AS String=""
		a = i
		If myReader.HasRows Then
			While myReader.Read()
				jenismemoxx = myReader("jenismemo").ToString()
			End While
		End If
		myConn.Close()
		...
		datastrxx = idxx & ",'" & nomemoxx & "','" & tglsu & "'," & jenisxx & ",'" & perihalxx & "','" & rubrikxx & "','" & tujuanxx & "','" & _
						tembusanxx & "','" & jenisundanganxx & "','" & namafilexx & "','" & namafolderxx & "','" & extens & "'," & statusbum & _
						",'" & writenama & "','" & writenpp & "','" & namaklpxx & "','" & namaklpq & "'"
...
%>
		...
		<td style="padding-left: 5px; padding-top: 5px;  font-family: Arial; font-size: 0.9em;text-align: center; vertical-align: top; line-height: 110%"><% Response.Write (jenismemoxx) %></td>
		<td style="padding-left: 5px; padding-top: 5px;  font-family: Arial; font-size: 0.9em;text-align: left; vertical-align: top; line-height: 110%"><% Response.Write (nomemoxx) %></td>
...
%>
		<td style="padding-left: 0px; padding-top: 5px;  font-family: Arial; font-size: 0.9em;text-align: center; vertical-align: top;">
			...
			<img id="uploadPreview<% Response.Write (i)%>" src="./images/delete4.png" OnClick="hapusmemo(<% Response.Write (idxx) %>);" height="26px" width="25px"/><br/>
		</td>
<%
		End If
%>
...
<form method="post" name="delfrm" id="delfrm" action="memodelgo.aspx" enctype="multipart/form-data">
	<input type="hidden" id="delmemoid" name="delmemoid">
</form>
