...
' Jenis verifikasi
'==================================================================================================================
	If jenisver.ToUpper()="VERIFIKASI ANALIS" Then
		statusup = 1
	Elseif jenisver.ToUpper()="VERIFIKASI PENGELOLA" Then
		statusup = 2
	Elseif jenisver.ToUpper()="VERIFIKASI PIM KEL" Then
		statusup = 3
	Elseif jenisver.ToUpper()="PERSETUJUAN PIMPINAN" Then
		statusup = 4	
	End If
...
'** Cek apakah data sudah ada
			SQLjo = "SELECT * FROM [pengadaanverifikasi] WHERE (pengadaanID=" & mohonid & " AND UPPER(jenisver)='" & jenisver & "' AND UPPER(status)='" & _
					status & "')"
...
'==================================================================================================================
			If dataexist=0 Then
				SQL0 = "INSERT INTO [pengadaanverifikasi] (pengadaanID, tgl, jenisver, status, keterangan, createduser, createddatetime) VALUES(" & _
						mohonid & ",'" & tglverx & "','" & jenisver & "','" & status & "','" & ket & "'," & idq & ",GETDATE())"
						'kcx.ToUpper() & "',1)"
'response.write(SQL0)			
				...
				SQL1 = "UPDATE pengadaan SET nostep=" & statusup & ", statusreject=0 WHERE ID=" & mohonid
				myCmd.CommandText = SQL1
				myConn.Open()
				myReader = myCmd.ExecuteReader()
				myConn.Close()
				
				If status.ToUpper()="REJECT" Then
					SQL1 = "UPDATE pengadaan SET statusreject=1 WHERE ID=" & mohonid
					myCmd.CommandText = SQL1
					myConn.Open()
					myReader = myCmd.ExecuteReader()
					myConn.Close()				
...
<meta http-equiv="refresh" content="0;url=permohonanedit.aspx?padaid=<% response.write(mohonid) %>" />
</body>
</html>