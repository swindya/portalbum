...
function hapussurat(id)
{
	if (confirm("Apakah anda ingin hapus data ?") == true) 
	{
		...
		if (tenure == "123") {
			document.getElementsByName("delsuratid")[0].value = id;
...
<script>
function editdatax(id, nosurat, tgl, tglcatat, jenis, perihal, tujuan, dari, tembusan, namafile, namafolder, extens)
{
//alert(id + "-" + jenisundangan);
	document.getElementById("databarutab").style.display = "none";
	...
	document.getElementsByName("editsuratid")[0].value = id;
	document.getElementsByName("edittgl")[0].value = tglcatat;
	document.getElementsByName("edittglsurat")[0].value = tgl;
	document.getElementsByName("editnomor")[0].value = nosurat;
	document.getElementsByName("editdari")[0].value = nosurat;
...
<%
	...
	Dim tglbikinxx as String
	Dim rubrikxx as String
	Dim nomorxx as String
	Dim darixx as String
	...
	Dim jenisc as String = ""
	Dim jenissekrestr as String = ""
...
<%
			If NOT Request.QueryString("jenis") is Nothing Then
				jenissurat = Request.QueryString("jenis")
			End If
			If jenissurat="" Then
				If NOT Request.Form("jenisur") is Nothing Then
					jenissurat = Request.Form("jenisur")
				End If			
			End If
...
<%
			...
			jenissekrestr = " AND UPPER(jenisstr)='" & jenissurat.ToUpper() & "'"
'If perihal IsNot Nothing || NOT perihal Is Nothing
			...
			If tgl1.length() > 1 AND String.IsNullOrEmpty(tgl2) Then
				...
				tglstr = " AND tglsurat='" & tgl10 & "'"
			Elseif String.IsNullOrEmpty(tgl1) AND tgl2.length() > 1 Then
				tglarr = tgl2.Split("-")
				tgl20 = tglarr(2) & "-" & tglarr(1) & "-" & tglarr(0)
				tglstr = " AND tglsurat='" & tgl20 & "'"
			Elseif tgl1.length()>1 AND tgl2.length()>1 Then
				tglarr = tgl1.Split("-")
				tgl10 = tglarr(2) & "-" & tglarr(1) & "-" & tglarr(0)
				tglarr = tgl2.Split("-")
				tgl20 = tglarr(2) & "-" & tglarr(1) & "-" & tglarr(0)
				tglstr = " AND tglsurat BETWEEN '" & tgl10 & "' AND '" & tgl20 & "'"
			End If

'============================================================================
' Data SURAT
'============================================================================
			SQL0 = "SELECT ID FROM [surat_sekretaris] WHERE (ID>0" & perihalstr & tglstr & jenissekrestr & ") ORDER BY tglsurat DESC"
'response.write(SQL0)
...
<%
		If jenissurat="SURAT MASUK" Then
			jenisc = "Surat Masuk"
%>
			<FONT face="Arial" color="#555555">DAFTAR SURAT MASUK</FONT>
<%
		Elseif jenissurat="SURAT KELUAR" Then
			jenisc = "Surat Keluar"
%>
			<FONT face="Arial" color="#555555">DAFTAR SURAT KELUAR</FONT>
<%
		Elseif jenissurat="MEMO MASUK" Then
			jenisc = "Memo Masuk"
%>
			<FONT face="Arial" color="#555555">DAFTAR MEMO MASUK</FONT>
<%		
		Elseif jenissurat="MEMO KELUAR" Then
			jenisc = "Memo Keluar"
%>
			<FONT face="Arial" color="#555555">DAFTAR MEMO KELUAR</FONT>		
<%
		Elseif jenissurat="NOTA INTERN MASUK" Then
			jenisc = "Nota Intern Masuk"
%>
			<FONT face="Arial" color="#555555">DAFTAR NOTA INTERN MASUK</FONT>
<%		
		Elseif jenissurat="NOTA INTERN KELUAR" Then
			jenisc = "Nota Intern Keluar"
%>
			<FONT face="Arial" color="#555555">DAFTAR NOTA INTERN KELUAR</FONT>
<%		
		End If
%>
		</td>
...
<form method="post" name="barufrm" id="barufrm" action="suratsekreaddgo.aspx" enctype="multipart/form-data">
<table id="databarutab" style="text-align: left; margin-left: 270px; margin-right: auto; margin-top: 0; display: none" width="950px" border="0" cellspacing="0" cellpadding="0">
	<tr height="0px">
		<td colspan="4" style="padding-left: 20px; text-align: left; font-size: 1.0em">
		</td>
	</tr>
	...
	<tr height="35px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle;" width="600px">
			<FONT face="Arial" color="#000">
			<select STYLE="color: #000000; background-color: #ffffff; font-size: 0.9em; width: 200px; height: 28px;" id="soflow11" name="barujenis" data-placeholder="Jenis" tabindex="1"> 
				<option value="" >&nbsp;&nbsp;</option>
				<option value="SURAT MASUK" 
				<%
					If jenisc="Surat Masuk" Then
						response.write("selected")
					End If
				%>
				>SURAT MASUK</option>
				<option value="MEMO MASUK"
				<%
					If jenisc="Memo Masuk" Then
						response.write("selected")
					End If
				%>							
				>MEMO MASUK</option>
				<option value="NOTA INTERN MASUK"
				<%
					If jenisc="Nota Intern Masuk" Then
						response.write("selected")
					End If
				%>				
				>NOTA INTERN MASUK</option>
				<option value="">-----------------------------</option>
				<option value="SURAT KELUAR"
				<%
					If jenisc="Surat Keluar" Then
						response.write("selected")
					End If
				%>				
				>SURAT KELUAR</option>
				<option value="MEMO KELUAR"
				<%
					If jenisc="Memo Keluar" Then
						response.write("selected")
					End If
				%>				
				>MEMO KELUAR</option>
				<option value="NOTA INTERN KELUAR"
				<%
					If jenisc="Nota Intern Keluar" Then
						response.write("selected")
					End If
				%>				
				>NOTA INTERN KELUAR</option>
			</select>			
			</FONT>
		</td>
	...
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
				<input style="font-family: Arial; font-size: 0.9em; width: 200px; height: 18px" type="text" id="barunomor" name="barunomor" class="textbox" tabindex="3">	
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em">										
		</td>
	...
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
				<input style="padding-left:10px; font-size: 0.9em; background-color: #FFF; border:1px solid #ababab; height: 24px; width: 100px" autocomplete="off" type="text" name="barutglsurat" id="barutglsurat" class="tcal"  STYLE="color: #000000; background-color: #FFFFFF;" tabindex="1"/>
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em">										
		</td>
	...
	<!--tr height="35px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: top">
			<FONT face="Arial" color="#000">
			<select STYLE="color: #000000; background-color: #ffffff; font-size: 0.9em; width: 130px" id="soflow12" name="barupengirim" data-placeholder="Jenis" tabindex="5"> 
				<option value="" >&nbsp;&nbsp;</option>
<%
...
%>
	...
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
				<input style="font-family: Arial; font-size: 1.0em; width: 640px; height: 20px;" type="text" id="barudari" name="barudari" class="textbox" tabindex="6">	
			</FONT>
		</td>
	...
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
				<input style="font-family: Arial; font-size: 0.9em; width: 640px; height: 20px;" type="text" id="barutujuan" name="barutujuan" class="textbox" onchange="getnama(this.value);" tabindex="7">
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.0em">										
		</td>
	...
	<tr height="65px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: top">
			<FONT color="#000">
				<textarea STYLE="font-family: Arial; height: 50px; width: 640px; color: #000000; padding-left: 5px; padding-middle: 5px; padding-bottom: 5px; border:1px solid #ababab; font-weight: normal; font-size:0.9em; line-height: 110%;" id="baruperihal" name="baruperihal" tabindex="8"></textarea>
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em">										
		</td>
	...
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
				<input style="font-family: Arial; font-size: 0.9em; width: 640px; height: 20px;" type="text" id="barutembusan" name="barutembusan" class="textbox" onchange="getnama(this.value);" tabindex="9">
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em">										
		</td>
...
<%
'response.write("-----------------------------------" & namauserx & strDate)
%>

<form method="post" name="editfrm" id="editfrm" action="suratsekreeditgo.aspx" enctype="multipart/form-data">
<table id="dataedittab" style="text-align: left; margin-left: 270px; margin-right: auto; margin-top: 0; display: none" width="1050px" border="0" cellspacing="0" cellpadding="0">
	...
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle;" width="700px">
			<FONT face="Arial" color="#000">
			<select STYLE="color: #000000; background-color: #ffffff; font-size: 0.9em; width: 200px" id="soflow21" name="editjenis" data-placeholder="Jenis" tabindex="1"> 
				<option value="" >&nbsp;&nbsp;</option>
				<option value="SURAT MASUK">SURAT MASUK</option>
				<option value="MEMO MASUK">MEMO MASUK</option>
				<option value="NOTA INTERN MASUK">NOTA INTERN MASUK</option>
				<option value="">-----------------------------</option>
				<option value="SURAT KELUAR">SURAT KELUAR</option>
				<option value="MEMO KELUAR">MEMO KELUAR</option>
				<option value="NOTA INTERN KELUAR">NOTA INTERN KELUAR</option>
			</select>			
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em" width="100px">										
	...
	<!--tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
			<select STYLE="color: #000000; background-color: #ffffff; font-size: 0.9em; width: 130px" id="soflow11" name="edittahun" data-placeholder="Jenis" tabindex="2"> 
				<option value="" >&nbsp;&nbsp;</option>
				<option value="<% Response.Write(tahunwingi)%>"><% Response.Write(tahunwingi)%></option>
				<option value="<% Response.Write(tahuniki)%>"><% Response.Write(tahuniki)%></option>
				<option value="<% Response.Write(tahunsesuk)%>"><% Response.Write(tahunsesuk)%></option>
	...
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
				<input style="font-family: Arial; font-size: 0.9em; width: 200px; height: 20px" type="text" id="editnomor" name="editnomor" class="textbox" tabindex="3">	
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em">										
		</td>
	...
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
				<input style="padding-left:10px; font-size: 0.9em; background-color: #FFF; border:1px solid #ababab; height: 24px; width: 100px" autocomplete="off" type="text" name="edittglsurat" id="edittglsurat" class="tcal"  STYLE="color: #000000; background-color: #FFFFFF;" tabindex="1"/>
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em">										
		</td>
	...
	<!--tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
			<select STYLE="color: #000000; background-color: #ffffff; font-size: 0.9em; width: 130px" id="soflow12" name="editpengirim" data-placeholder="Jenis" tabindex="5"> 
				<option value="" >&nbsp;&nbsp;</option>
<%
			for b=1 to jmlrubrik
%>
...
%>
	...
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
				<input style="font-family: Arial; font-size: 0.9em; width: 640px; height: 20px" type="text" id="editdari" name="editdari" class="textbox" tabindex="6">	
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em">										
		</td>
	...
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
				<input style="font-family: Arial; font-size: 0.9em; width: 640px; height: 20px" type="text" id="edittujuan" name="edittujuan" class="textbox" onchange="getnama(this.value);" tabindex="7">
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em">										
		</td>
	...
	<tr height="65px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: top">
			<FONT face="Arial" color="#000">
				<textarea STYLE="font-family: Arial; height: 50px; width: 640px; color: #000000; padding-left: 5px; padding-middle: 5px; padding-bottom: 5px; border:1px solid #ababab; font-weight: normal; font-size:0.9em; line-height: 110%;" id="editperihal" name="editperihal" rows="3" cols="70" tabindex="8"></textarea>
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.0em">										
		</td>
	...
	<tr height="38px">
		...
		<td style="padding-left: 0px; text-align: left; vertical-align: middle">
			<FONT face="Arial" color="#000">
				<input style="font-family: Arial; font-size: 0.9em; width: 640px; height: 20px" type="text" id="edittembusan" name="edittembusan" class="textbox" onchange="getnama(this.value);" tabindex="9">
			</FONT>
		</td>
		<td style="padding-left: 0px; text-align: center; font-size: 1.2em">										
		</td>
	...
	<tr height="38px">
		<td style="padding-left: 0px; padding-middle: 5px; text-align: left; font-size: 0.9em; vertical-align: middle">
			<FONT face="Arial" color="#000">
				Attach Dok
			</FONT>											
		</td>
		<td style="padding-left: 0px; padding-middle: 5px; text-align: center; font-size: 0.9em; vertical-align: middle">
			<FONT face="Arial" color="#000">
				 :
			</FONT>											
		</td>
		<td style="padding-left: auto; padding-middle: 5px; text-align: left; font-size: 1.2em; vertical-align: middle">
			<table id="file1" style="text-align: left; margin-left: 0px; margin-right: auto; margin-middle: auto;" border="0" cellspacing="0" cellpadding="0">
				<tr height="42px">
					<td style="padding-left: 0px; padding-middle: 0px; vertical-align: middle; text-align: left; font-size: 0.9em">
						<FONT face="Arial" color="#000">
							<input id="uploadImage1" name="uploadImage1" type="file" onchange="PreviewImage(1);" onclick="resetpic();" tabindex="6"  />
						</FONT>
					</td>
					<td style="padding-middle: 5px; padding-left: auto; text-align: right; font-size: 1.2em" width="40px">
						<FONT face="Arial" color="#000">
	...
	<tr height="30px">
		<td colspan="4">
			<input type="hidden" id="editsuratid" name="editsuratid">
...
<form method="post" name="filterfrm" id="filterfrm" action="suratsekre.aspx" enctype="multipart/form-data">
<table id="filtertab" style="text-align: left; margin-left: 270px; margin-right: auto; margin-top: auto;" width="1150px" border="0" cellspacing="0" cellpadding="0" bgcolor="#CAFFEB">
	...
	<tr height="20px">
		<td colspan="3" style="padding-left: 20px; text-align: left;">
			...
			<input type="hidden" id="jenisur" name="jenisur" value="<% response.write(jenissurat) %>">
		</td>
	</tr>
</table>
</form>
...
<table id="barubtntab" style="margin-left: 270px;" width="1150px" align="center" cellpadding="2" cellspacing="2" border="0" bgcolor="#EEE" >
	<tr height="38px" align="left">
		...
		<td style="font-family: Arial; font-size: 0.9em;text-align: left;" width="140px">
			<a href="#" class="css-button-4" style="text-decoration: none; font-size: 1.1em" onclick="exportcsv();">
		...
		<td style="font-size: 0.9em;text-align: left; vertical-align: bottom" width="140px">
...
<table id="datatab" style="margin-left: 270px;" width="1150px" align="center" cellpadding="2" cellspacing="2" border="0" bgcolor="#FAFAFA" >
	<tr height="42px" align="left" style="background-color:#D5D5D5;color:Black;" >
		...
		<td style="font-family: Arial; font-size: 0.9em;text-align: center;" width="100px"><b>TGL SURAT</b></td>            
		<td style="font-family: Arial; font-size: 0.9em;text-align: center;" width="130px"><b>JENIS</b></td>
		<td style="font-family: Arial; font-size: 0.9em;text-align: center;" width="140px"><b>NO SURAT</b></td>
		<td style="font-family: Arial; font-size: 0.9em;text-align: center;" width="340px"><b>PERIHAL</b></td>
		<td style="font-family: Arial; font-size: 0.9em;text-align: center;" width="250px"><b>TUJUAN</b></td>
...
Try
	...
	for i = startval to endval
		ii = ii + k
		bb = bb + 1
		SQL1 = "SELECT * FROM [surat_sekretaris] WHERE ID=" & idz(i)			
		...
		If myReader.HasRows Then
			While myReader.Read()
				...
				jenisxx = myReader("jenisstr").ToString()
				nomorxx = myReader("nomor").ToString()
				darixx = myReader("dari").ToString()
				...
				tglbikinxx = myReader("createddate").ToString()
		...
		If tglxx.length > 4 Then
			...
			tglsu = format(dt, "d-M-yyyy").ToString()
		End If
		...
		tglsub = ""
		If tglbikinxx.length > 4 Then
			dt1 = Convert.ToDateTime(tglbikinxx)
			tglsub = format(dt1, "d-M-yyyy").ToString()
			'tglsub = tglbikinxx.ToString()
		End If
		...
		datastrxx = idxx & ",'" & nosuratxx & "','" & tglsu & "','" & jenisxx & "','" & perihalxx & "','" & rubrikxx & "','" & tujuanxx & "','" & _
						tembusanxx & "','" & jenisundanganxx & "','" & namafilexx & "','" & namafolderxx & "','" & extens & "'"
						
		datastrxx = idxx & ",'" & nosuratxx & "','" & tglsu & "','" & tglsub & "','" & jenisxx & "','" & perihalxx & "','" & tujuanxx & "','" & _
						darixx & "','" & tembusanxx & "','" & namafilexx & "','" & namafolderxx & "','" & extens & "'"
...
<%
		End If
%>
		<td style="padding-right: 5px; padding-top: 5px;  font-family: Arial; font-size: 0.9em;text-align: right; vertical-align: top; line-height: 110%"><% Response.Write (startval+bb) %></td>
		...
		<td style="padding-left: 5px; padding-top: 5px;  font-family: Arial; font-size: 0.9em;text-align: center; vertical-align: top; line-height: 110%"><% Response.Write (jenisxx) %></td>
		<td style="padding-left: 5px; padding-top: 5px;  font-family: Arial; font-size: 0.9em;text-align: left; vertical-align: top; line-height: 110%"><% Response.Write (nosuratxx) %></td>
		...
		<td style="padding-left: 0px; padding-top: 5px; font-family: Arial; font-size: 0.9em;text-align: center; vertical-align: top;">
			...
			<img id="uploadPreview<% Response.Write (i)%>" src="./images/delete4.png" OnClick="hapussurat(<% Response.Write (idxx) %>);" height="26px" width="25px"/><br/>
		</td>
	</tr>
<%
	next
...
<form method="post" name="delfrm" id="delfrm" action="suratsekredelgo.aspx" enctype="multipart/form-data">
	<input type="hidden" id="delsuratid" name="delsuratid">
</form>
