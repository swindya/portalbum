...
<%	
    'Using workBook As New XLWorkbook(FileUpload1.PostedFile.InputStream)
        ...
        'Read the first Sheet from Excel file.
			...
			namafile0 = folder1 & "regvOUT.xls"
			namafileout = folder1 & "regvOUT.xls"
...
'response.write(SQLex & "<br>")
			...
			If myReader.HasRows Then
				While myReader.Read()
					...
					jenisxx(a) = myReader("jenis").ToString()
					If jenisxx(a)="DI" Then
						jenisstrxx(a) = "DEBET INTERN"
					Else If jenisxx(a)="NK" Then
						jenisstrxx(a) = "NOTA KREDIT"
					Else If jenisxx(a)="ND" Then
						jenisstrxx(a) = "NOTA DEBET"
					End If
					klpxx(a) = Val(myReader("klp").ToString())
					rubrikxx(a) = myReader("rubrik").ToString()
					perihalxx(a) = myReader("perihal").ToString()
					norekdebetxx(a) = myReader("norekdebet").ToString()
					norekkreditxx(a) = myReader("norekkredit").ToString()
...
'response.write(jmlsurat & "<br>")
			For a = 1 to jmlsurat
				connExcelw.Open()
				cmdExcelw.Connection = connExcelw
				sqlout = "INSERT INTO [Sheet1$] ([NO], [NOMOR], [NOURUT], [TGL], [JENIS], [PERIHAL], [NOREK DEBET], [NOREK KREDIT], [NOMINAL]) " & _
						"VALUES(" & a & ",'" & nosuratxx(a) & "'," & nourutxx(a) & ",'" & tglxx(a) & "','" & jenisstrxx(a) & "','" & perihalxx(a) & "','" & _
						norekdebetxx(a) & "','" & norekkreditxx(a) & "'," & nominalxx(a) & ")"
...
<%
	namafileout = "./DOC/register/Temp/regvOUT.xls"
%>
