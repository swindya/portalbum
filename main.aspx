...
<%
	...
	'If NOT Session("username") is nothing OR NOT Session("namaklp") is nothing Then
		statusmain = 1
...
<%			
			'myConn = New SqlConnection("Data Source=DESKTOP-S7N8DF9;Initial Catalog=PORTALBUMGO;User ID=sa;Password=sa;")
...
<%
				...
				If a=1 Then
					...
					SQL1 = "SELECT * FROM [klp] WHERE (klp_id=" & klpq & ")"
					'SQL1 = "SELECT * FROM [klp] WHERE (ID=" & klpq & ")"					
					myCmd.CommandText = SQL1
					myConn.Open()
					myReader = myCmd.ExecuteReader()
					...
					If myReader.HasRows Then
						While myReader.Read()
							namaklpq = myReader("klp_nama").ToString()
							namakel = namaklpq
						End While
					End If
					myConn.Close()
					
					SQL1 = "SELECT * FROM [unit] WHERE (u_klp=" & klpq & ")"
					'SQL1 = "SELECT * FROM [klp] WHERE (ID=" & klpq & ")"					
					myCmd.CommandText = SQL1
					myConn.Open()
					myReader = myCmd.ExecuteReader()
...
<%
			...
			If String.IsNullOrEmpty(namauserq) OR namauserq="" OR String.IsNullOrEmpty(namaklpq) Then
%>
...
<%			
	...
	If Not namauserq is Nothing Then
...
'response.write("---------------------------------------++" & SQL0 + divq)
'response.write(namakel)
'response.write(Session("namaklp") & "--")
...
<table style="text-align: left; margin-left: 280px; margin-right: auto; margin-top: auto;" width="75%" border="0" cellspacing="0" cellpadding="0">
	<tr height="20px">
		<td style="padding-left: 0px; text-align: left; font-size: 1.8em" width="610px">
			<FONT face="Arial" color="#000">SELAMAT DATANG DI &nbsp;<b><i>PORTAL BUM</i></b></FONT>
		</td>
